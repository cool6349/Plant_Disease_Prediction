...
export function TomatoDisease() {
  ...
  const handleUpload = async () => {
    if (!selectedFile) return alert("Please .")
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <h4 className="font-medium text-green-800 mb-2">Disease Detected: {result.class}</h4>
                    <p className="text-green-700 text-sm">
                      Confidence: {(result.confidence * 100).toFixed(2)}% - This is the predicted disease for your {plantName?.toLowerCase()}.
                    </p>
                  </div>
                ...
                {diseases.map((disease, index) => (
                  <div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex space-x-4">
                      <img src={disease.image || "/placeholder.svg"} alt={disease.name} className="w-16 h-16 object-cover rounded-lg flex-shrink-0" />
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center justify-between">
                          <h4 className="font-medium text-gray-900">{disease.name}</h4>
                          <span className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${getSeverityColor(disease.severity)}`}>
